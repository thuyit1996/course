'use client';
import { InvokeTimmer } from '@/types/exam';
...
interface CountdownTimerProps {
  ...
  alertFinish?: () => void;
  duration?: number;
  status: 'starting' | 'pausing' | 'ending';
}

const CountDown = forwardRef<InvokeTimmer, CountdownTimerProps>(
  ({ duration, status, submitExample, onCancel, onUpdateStatus, onStop, isReview = false, examId, alertFinish }, ref: Ref<InvokeTimmer>) => {
    const numberDuration = (duration ?? 20) * 60; // 20 minutes
    const radius = 90;
    const strokeWidth = 15;
    const circumference = 2 * Math.PI * radius;
    const [, setIsPaused] = useState(false);
    const [timeLeft, setTimeLeft] = useState(numberDuration);
    ...
    const offset = circumference - (timeLeft / numberDuration) * circumference;
    const onSubmitExam = () => {
