  ...
  return (
    <main className="md:ml-[288px]">
      <div className='shadow rounded-3xl bg-white  h-[calc(100vh-2rem)]'>
        <div className='flex justify-between px-6 py-4'>
          <div className='flex items-center'>
            ...
            <div className='border rounded-2xl ml-3 border-gray-200 text-[#2c2c2c] text-sm text-center px-2 py-1 text-medium'>{data?.total} {data?.total as number > 1 ? 'exams' : 'exam'}</div>
          </div>
          <Button variant='primary' onClick={openModal} startIcon={<PlusIcon className="fill-rose-600" />}>Create Exams</Button>
        </div>
        <div className=''>
      ...
      <Pagination onChange={(pageIndex: number) => {
        router.push(`/admin/exams?${createQueryString(examPramConfig, { ...decodedParams, pageIndex })}`)
