...
import BellIcon from '@/public/images/icons/CallBell.svg'
...
import { createAttendance, getAttendanceDetail } from '@/api/admin/fetches';
...
const AttendanceDetail = dynamic(() => import('@/components/attendance-detail'), { ssr: false });
const dropdownItems = [
  { title: 'Completed', value: 'Completed' },
  { title: 'Pending', value: 'Pending' },
  { title: 'Missing', value: 'Missing' },
]
type HomeWorkStatus = 'Completed' | 'Missing' | 'Pending';
const Students = ({ gradeId }: { gradeId: string }) => {
  const router = useRouter();
  const [selectUserId, setSelectUserId] = useState('');
  const attendanceDetailModal = useModal();
  ...
  const attendanceUsers = () => {
    const statusMap = keyBy(selectedUsers, 'id');

    const result = map(
      filter(data?.users, user => !('attendance' in user)),
      (user: User) => {
        const statusItem = statusMap[user.userId];

        const homeworkStatus = get(
          { 'Completed': 0, 'Pending': 1 },
          statusItem?.status,
          2
        );

        const status = statusItem ? (statusItem.selected ? 0 : 1) : 1;
    ...
    setModalMessage(`You are about to mark attendance for ${result?.length ? result?.length : '0'} students. Do you want to proceed?`)
    return result;
  }
  ...
  const modalAttendance = useModal();
  ...
  const onOpenDropdown = (userId: string) => {
    const existUser = selectedUsers.find(item => item.id === userId);
    console.log(existUser);
    if (existUser) {
      const selected = selectedUsers.map(item => {
        if (item.id !== userId) {
          console.log("here");
          return {
            ...item,
      ...
      setSelectedUsers(prev => {
        return [
          ...
          { id: userId, status: '' as HomeWorkStatus, isOpen: true, selected: false }
        ]
      })
    }
  }
  const onSelectUserHandle = (id: string, status: HomeWorkStatus) => {
    const existUser = selectedUsers.find(item => item.id === id);
    if (existUser) {
      const selected = selectedUsers.map(item => {
        return item.id === id ? { ...item, status, isOpen: false } : { ...item }
      })
      setSelectedUsers(selected);
    } else {
      setSelectedUsers(prev => {
        return [
          ...prev,
          { id, status, isOpen: false, selected: false }
        ]
      })
    }

  }
  const findUserInList = (id: string) => {
    return selectedUsers.find(item => item.id === id)
  }
  const onSelectedForAttendance = (selected: boolean, userId: string) => {
    const users = selectedUsers.map(item => item.id !== userId ? { ...item } : { ...item, selected })
    setSelectedUsers(users);
  }
  const onSelectedAllHandle = (selected: boolean) => {
  ...
  const isSelectedAll = useMemo(() => {
    return selectedUsers.length && selectedUsers?.filter(item => item.selected)?.length === selectedUsers?.length;
  }, [selectedUsers]);

  const updateAttendance = async (userId: string) => {
    setSelectUserId(userId);
    attendanceDetailModal.openModal();
  }
  return (
    <main className="md:ml-[288px]">
      <div className='shadow rounded-3xl bg-white  h-[calc(100vh-2rem)]'>
        <div className='flex justify-between px-6 py-4'>
          ...
          <div className='flex gap-x-3'>
            ...
            }>{data?.staff ? data?.staff.displayName : ''}</Button>
            <Button variant='secondary' disabled={!data?.users?.length} onClick={() => {
              modalAttendance.openModal();
              attendanceUsers();
            ...
            <Button variant='primary' onClick={openModal} startIcon={<PlusIcon className="fill-rose-600" />}>Add Student</Button>
          </div>
        </div>
        <div className='h-[70vh]'>
          <div className="overflow-hidden border-t border-gray-100 bg-white h-full">
            <div className="max-w-full overflow-x-auto overflow-y-auto h-full scroll-hidden">
                ...
                <Table>
                  <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                    <TableRow>
                      <TableCell
                        isHeader
                        className="px-5 py-3 flex text-[#757575] text-xs font-semibold text-start min-w-[200px]"
                      >
                        <Checkbox onChange={(status) => onSelectedAllHandle(status)} disabled={!selectedUsers?.find(item => item.status)} checked={!!isSelectedAll} className="mr-2 cursor-pointer" label='Name' textClass='text-[#757575] text-xs font-semibold' />
                      </TableCell>
                      <TableCell
                        isHeader
                      ...
                      <TableCell
                        isHeader
                        className="px-5 py-3 text-[#757575] text-xs font-semibold  text-start min-w-[150px]"
                      >
                        Homework
                      </TableCell>
                      <TableCell
                      ...
                      <TableCell
                        isHeader
                        className="px-5 py-3 text-[#757575] text-xs font-semibold  text-start min-w-[200px]"
                      >
                        Address
                      </TableCell>
                      <TableCell
                    ...
                    {data?.users?.length ? <>
                      {data?.users.map((user, index) => (
                        <TableRow key={user.userId}>
                          <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 w-[200px]">
                            {/* a */}
                            <Checkbox onChange={value => onSelectedForAttendance(value, user.userId)} disabled={!(findUserInList(user.userId)?.status)} checked={!!findUserInList(user.userId)?.selected as boolean} className="mr-2 cursor-pointer" label={user.displayName} textClass='text-gray-500  text-theme-sm dark:text-gray-400' />
                          </TableCell>
                          <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 capitalize">
                            {user.attendance ?
                              <div className='border border-gray-200 flex rounded-md justify-center items-center px-1.5 py-[2px]'>
                                <div className={`${user.attendance === "PRESENT" ? 'bg-[#17B26A]' : 'bg-[#B3B3B3]'} rounded-full w-1.5 h-1.5 mr-1.5`}></div>
                                <span>{user?.attendance?.toLowerCase()}</span>
                              </div>
                              : <>-</>}
                          </TableCell>
                          <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 capitalize w-[150px]">
                            {user?.homework ? user?.homework?.toLowerCase() : <div className="relative cursor-pointer select-none hover:text-rose-600">
                              <div className='flex' onClick={() => {
                                onOpenDropdown(user.userId);
                              }}>
                                <span>{findUserInList(user.userId)?.status ? findUserInList(user.userId)?.status : 'Not Started'}</span>
                                <svg
                                  className={`stroke-gray-500 dark:stroke-gray-400 transition-transform duration-200 ml-2 ${findUserInList(user.userId)?.isOpen ? 'rotate-180' : ''}`}
                                  width="18"
                                  height="20"
                                  viewBox="0 0 18 20"
                                  fill="none"
                              ...
                              <Dropdown
                                isOpen={findUserInList(user.userId)?.isOpen as boolean}
                                onClose={closeDropdown}
                                className="w-40 p-2 absolute right-0"
                              >
                                {dropdownItems.map((dropdown) => {
                                  return (
                                    <DropdownItem
                                      key={dropdown.value}
                                      onItemClick={() => onSelectUserHandle(user.userId, dropdown.value as HomeWorkStatus)}
                                      className={`flex w-full font-normal text-left text-gray-500 rounded-lg hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300 ${findUserInList(user.userId)?.status === dropdown.value ? 'text-rose-600 bg-rose-100' : ''}`}
                                    >
                                      {dropdown.title}
                                    </DropdownItem>
                                  )
                          ...
                          <TableCell className="px-5 py-3 text-gray-500 text-theme-sm dark:text-gray-400 line-clamp-2">
                            {user.address ?? '-'}
                          </TableCell>
                          <TableCell className="px-5 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                          ...
                          <TableCell className="px-5 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                            <div className="flex gap-4">
                              ...
                              <BellIcon className="cursor-pointer" onClick={() => updateAttendance(user.userId)} />
                              <LockIcon className="cursor-pointer" />
                              <PasswordIcon className="cursor-pointer" />
      ...
      {isOpen && <AddUser isOpen={isOpen} closeModal={(isSuccess?: boolean) => {
        closeModal();
        if (isSuccess) {
          router.push(`/admin/grade/${gradeId}?${createQueryString(studentPramConfig, { ...decodedParams, pageIndex: 0 })}`)
        }
      }} role="USER" gradeId={gradeId} />}
      <ConfirmModal isOpen={modalAttendance.isOpen} closeModal={modalAttendance.closeModal} handleSave={handleAttendance}
        buttonLabel={['Cancel', 'Submit Now']}
        title={`Confirm Attendance for Class ${searchParams.get('grade')}`}
