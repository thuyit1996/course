import { getQuizTestList } from '@/api/exam/fetches';
...
const QuizTestPage = async () => {
    // const topicResp = await getAllTopic();
    const quizTestResp = await getQuizTestList();
    console.log('quizTestResp', JSON.stringify(quizTestResp))
    return (
        <div className="mt-[104px] 4xl:px-[175px] 2xl:px-[150px] lg:px-[100px] md:px-6 gap-6 px-4">
            <div className="flex items-center w-full">
                ...
                <div className='border rounded-2xl  border-gray-200 text-[#2c2c2c] text-sm text-center px-2 py-1 text-medium'>{quizTestResp?.responseData?.exams?.length || 0} tests</div>
            </div>

            {quizTestResp?.responseData?.exams?.length ?
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 4xl:grid-cols-5 gap-4 mb-10">
                    {quizTestResp?.responseData?.exams.map((item) => {
                        return <div className="rounded-2xl p-6 relative bg-indigo-800 bg-[url('/images/grid-image/test.png')] bg-cover bg-center" key={item.id} data-aos="fade-up">
                            ...
                            <div className='mt-10 flex justify-center items-center'>
                                {item.score !== undefined ? <>
                                    <Link href={`/quiz-test/review/${item.id}`} className='w-1/2 mr-2.5'>
                                        <Button variant='outline' className='w-full'>Review</Button>
                                    </Link>
                                </>
                                    :
                                    <Link href={`/quiz-test/start/${item.id}`} className='w-full'>
